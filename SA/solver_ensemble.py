...
def main():
    if len(sys.argv) < 6:
        print("Usage: python solver_ensemble.py <warehouse> <obstacles> <ceiling> <types_of_bays> <output>")
        sys.exit(1)
    ...
    # Distribute available cores roughly equally between ortho and sat solvers
    num_ortho = num_cores // 2
    num_sat = num_cores - num_ortho
    ...
    print(f"Starting Ensemble with {num_ortho} Orthogonal and {num_sat} SAT solvers...", file=sys.stderr)
    ...
    if best_name:
        print(f"Ensemble Winner: {best_name} with Q={best_q:.2f}", file=sys.stderr)
        shutil.copy2(results[best_name]['file'], final_out)
    else:
        print("Ensemble Failed: No valid output found.", file=sys.stderr)
