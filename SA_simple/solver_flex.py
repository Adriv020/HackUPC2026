            ...
            if n_active > 0:
                ...
                if n_active > 1 and _random() < 0.5:
                    ...
                    if ref_idx in state.active:
                        ref = state.bays[ref_idx]
                        # Don't try adjacent with arbitrary rotations here to save time, use ortho
                        for rot in [orot, 0, 90]:
                            w = bt[BT_W]; d = bt[BT_D] + bt[BT_G]
                            cw = d if rot == 90 else w
                            cd = w if rot == 90 else d
                            trials = [
                                (ref[PB_X2], ref[PB_Y1]),
                                (ref[PB_X1] - cw, ref[PB_Y1]),
                                (ref[PB_X1], ref[PB_Y2]),
                                (ref[PB_X1], ref[PB_Y1] - cd),
                            ]
                            for tx, ty in trials:
                                if state.feasible(bt, tx, ty, rot):
                ...
                if not moved:
                    for _ in range(8):
                        if _random() < 0.6:
                            rot = _random() * 360.0  # Nudge rotation!
                            if rot < 0: rot += 360
                            if rot >= 360: rot -= 360
                        else:
                            rot = _random() * 360.0
